#include "SafeListenerStrategy.h"

void SafeListenerStrategy::addListener(std::string eventName, std::function<void(Event&)> listener)
{
...
void SafeListenerStrategy::removeListener(std::string eventName, std::function<void(Event&)> listener)
{
	// not really working at the moment
}

void SafeListenerStrategy::fireEvent(Event& event)
{
